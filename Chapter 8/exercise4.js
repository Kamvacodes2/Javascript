...
// 10. Return the value of the newly updaated string with capitalized words that can
// then be output into the console.

let mixedCaseString = "thIs will be capiTalized for each word";
// let capitalizedWords = []
function changeString(string) {
    let newString = string.toLowerCase(); // this will be capitalized for each word (1 item) 
    newString = newString.split(" ") // ['this', 'will', 'be', 'capitalized' ] 7 items separate
    let capitalizedWords = [];
    newString.forEach(element => {
      let cappedFirstLetter =  element.slice(0,1); //this - t
      cappedFirstLetter = cappedFirstLetter.toUpperCase(); // t = T
      let remainingLetters = element.slice(1) // this - his
      capitalizedWords.push((cappedFirstLetter.concat(remainingLetters))) // T + his = This, Will
    });
    return capitalizedWords.join(" ")
};

console.log(changeString(mixedCaseString));
